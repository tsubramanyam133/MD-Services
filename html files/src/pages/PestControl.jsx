...
import { defaultPestControl } from "../data/defaultServices";

export default function PestControl() {
  const [items, setItems] = useState(defaultPestControl);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        const q = query(collection(db, "pest-control"), orderBy("order", "asc"));
    ...
    <div>
      <header style={{ background: "var(--secondary-color)", color: "white", padding: "120px 0 60px", textAlign: "center" }}>
        <div className="container">
          <h1>Specialized Pest Control</h1>
          <p>Advanced treatments for Termites, Cockroaches, and more.</p>
        </div>
      </header>
            ...
            {items.map((item, index) => (
              <div key={item.id || index} className="pest-single-card">
                ...
                <div className="card-desc">
                  <p>{item.description || "Professional pest solution treatment."}</p>
                </div>
              </div>
            ))}
          </div>
